...
const settings = definePluginSettings({
    ...
    appendOriginalText: {
        type: OptionType.BOOLEAN,
        description: "Original text will be added on a new line as a spoiler",
        default: true
    }
});
...
const presendObject: SendListener = async (channelId, msg) => {
    ...
    if (settings.store.appendOriginalText) msg.content += `\n||${content}||`;
};
    ...
    if (container) {
        ...
        container.splice(idx + 1, 0,
            <Menu.MenuCheckboxItem
                id="vc-chinese-whispers"
                label="Chinese Whispers"
                checked={isEnabled}
                action={toggle}
            />
        );
